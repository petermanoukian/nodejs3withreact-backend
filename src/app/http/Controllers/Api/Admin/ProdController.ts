...
export class ProdController {
    ...
    static readonly instance = new ProdController(
        new ProdService(new ProdQuery(knex(knexConfig))),
    ...
    async list(req: Request, res: Response) {
        try {
            ...
            const orderByField = (body.orderByField ?? req.query.orderByField ?? "id") as string;
            const orderByDirection = (body.orderByDirection ?? req.query.orderByDirection ?? "desc") as "asc" | "desc";
            ...
            const searchOperator = (body.searchOperator ?? req.query.searchOperator ?? "OR") as "AND" | "OR";
            const searchMode = (body.searchMode ?? req.query.searchMode ?? "like") as "like" | "exact";
            ...
            if (catid) filters.catid = Number(catid);
            if (subcatid) filters.subcatid = Number(subcatid);

            console.log("filters:", filters);

            const result = await this.prodService.returnSearchPaginatedWithDetails(
            filters,
            orderByField,
            orderByDirection,
    ...
    async store(req: Request, res: Response) {
        ...
        const data: Partial<Prod> = { catid: Number(catid), subcatid: Number(subcatid), name, des, dess };
    ...
    async update(req: Request, res: Response) {
        ...
        if (!existing) return res.status(404).json({ message: "Prod not found" });
