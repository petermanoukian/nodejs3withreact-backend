// src/app/http/Request/Admin/ProdUpdateRequest.ts
import { Request } from "express";
import { ProdService } from "@Service/Action/Admin/ProdService";

export class ProdUpdateRequest {
  static async validate(req: Request, prodService: ProdService): Promise<{ valid: boolean; errors?: string[] }> {
    const errors: string[] = [];
    const id = Number(req.params.id);
    const { catid, subcatid, name, des, dess } = req.body;

    // Existence check
    const existing = await prodService.returnOneById(id);
    if (!existing) {
      errors.push("Prod not found.");
    }
    ...
    if (catid && subcatid && name) {
      const duplicate = await prodService.returnOne({ catid, subcatid, name });
